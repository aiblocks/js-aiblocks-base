...
The transaction will use the current sequence number of the given [Account](https://github.com/aiblocks/js-aiblocks-base/blob/master/src/account.js) object as its sequence number and increments
...
See [operation.js](https://github.com/aiblocks/js-aiblocks-base/blob/master/src/operation.js) for a list of possible operations you can add.
...
This will return a fully constructed [Transaction](https://github.com/aiblocks/js-aiblocks-base/blob/master/src/transaction.js).
The returned transaction will contain the sequence number of the source account. This transaction is unsigned. You must sign it before it will be accepted by the AiBlocks network.


```js
// AiBlocksBase.Network.usePublicNetwork(); if this transaction is for the public network
// Create an Account object from an address and sequence number.
var account=new AiBlocksBase.Account("GD6WU64OEP5C4LRBH6NK3MHYIA2ADN6K6II6EXPNVUR3ERBXT4AN4ACD","2319149195853854");

var transaction = new AiBlocksBase.TransactionBuilder(account, {
        fee: AiBlocksBase.BASE_FEE,
        networkPassphrase: Networks.TESTNET
    })
        // add a payment operation to the transaction
        .addOperation(AiBlocksBase.Operation.payment({
                destination: "GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTG2E5EDW",
                asset: AiBlocksBase.Asset.native(),
                amount: "100.50"  // 100.50 XLM
            }))
        // add a set options operation to the transaction
        .addOperation(AiBlocksBase.Operation.setOptions({
                signer: {
...
by passing a [memo](https://github.com/aiblocks/js-aiblocks-base/blob/master/src/memo.js) object when you construct the TransactionBuilder.
...
more, please consult the [multi-sig documentation](https://aiblocks.org/developers/learn/concepts/multi-sig.html).

You add signatures to a transaction with the `Transaction.sign()` function. You can chain multiple `sign()` calls together.

## `Keypair` class

`Keypair` object represents key pair used to sign transactions in AiBlocks network. `Keypair` object can contain both a public and private key, or only a public key.
